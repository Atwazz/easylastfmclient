...
extension ArtistsSearchResults: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        let queryContainer = try container.nestedContainer(keyedBy: QueryCodingKeys.self,
                                                           forKey: .searchQuery)
        let page = queryContainer.decodeUIntFromString(forKey: .page)
        let totalResults = container.decodeUIntFromString(forKey: .totalResults)
        let pageSize = container.decodeUIntFromString(forKey: .pageSize)
